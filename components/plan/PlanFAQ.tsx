...
export function buildPlanFAQItems(
  ...
  if (genericEntry?.copay_in_network != null && faqs.length < 6) {
    ...
    faqs.push({
      ...
      answer: `Based on federal plan benefit data, this plan charges ${copay === 0 ? '$0 (no cost)' : `$${copay}`} per month for generic drugs${deductible && deductible > 0 ? ' — confirm whether this copay applies before or after the deductible' : ''}. Preferred brand-name drugs and specialty drugs carry higher cost-sharing as shown in the cost-sharing grid above. Always verify current formulary placement and cost-sharing directly with ${plan.issuer_name}, as carriers can update formularies during the plan year.`,
    })
  }
